...
class OpenAiNetworkClient(token: String) {
	...
	@BetaOpenAI
	suspend fun suggestMonsterNames(monsterType: String): List<String> =
		coroutineScope {
	...
	private fun Iterable<CombatantModel>.commaSeparatedNameList(): String = joinToString(separator = ", ", transform = CombatantModel::name)

	private fun String.capitalizeWords(): String = this.split(" ").joinToString(" ") { it.capitalizeWord() }

	private fun String.capitalizeWord(): String = replaceFirstChar { if (it.isLowerCase()) it.titlecase() else it.toString() }
}
